...
const Gallery = (props) => {
  ...
  return (
    <>
      {viewImage && (
        <div className="pt-16 fixed  w-full h-full  bg-sky-700 bg-opacity-60  flex justify-center align-middle overflow-hidden z-40">
          <Swiper
                ...
                <SwiperSlide>
                  <div className="w-full flex ">
                    <button
                      ...
                      className="absolute right-0 rounded text-xl text-sky-300 bg-gray-700 px-3 pb-1 mt-10 mr-5"
                    >
                      x
                    </button>
                  </div>
      ...
      <div className="pt-20  ">
        ...
        <div className="w-fit mx-auto space-x-3 text-gray-300 pb-3">
          <button
            onClick={() => changeSeclectedCategory("all")}
            className="gallerySelector"
          >
            All
          </button>
          <button
            onClick={() => changeSeclectedCategory("debrebrhan")}
            className="gallerySelector"
          >
            Debrebrhan
          </button>
          <button
            onClick={() => changeSeclectedCategory("school")}
            className="gallerySelector"
          >
            School
          </button>
          <button
            onClick={() => changeSeclectedCategory("other")}
            className="gallerySelector"
          >
            Other
          </button>
        ...
        >
          <Masonry>
            {GalleryImages.filter(
              (image) =>
                categorySelected == "all" || image.category === categorySelected
            ).map((image) => (
              <div
                onClick={() => changeActiveImage(image.id)}
